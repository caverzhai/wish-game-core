...
// Find the LAST init(); in the file (the one at the end)
const lastInitIdx = js.lastIndexOf('init();');
if (lastInitIdx > -1) {
  js = js.substring(0, lastInitIdx) + pullRefreshCode + js.substring(lastInitIdx);
  fs.writeFileSync(path, js, 'utf8');
  console.log('Pull-to-refresh added at last init() position', lastInitIdx);
} else {
